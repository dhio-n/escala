...
def renderizar_gerador_escalas():
    ...
    with st.sidebar:
        ...
        # Configurações específicas para escalas 6x1
        st.subheader("🎯 Configurações Escalas 6x1")
        
        # Opção de folga em domingo
        folga_domingo_obrigatoria = st.radio(
            "Regra de Domingo:",
            options=[
                ("obrigatoria", "1 domingo por mês (obrigatório)"),
                ("negociacao", "1 domingo a cada 7 semanas (negociação coletiva)")
            ],
            format_func=lambda x: x[1],
            help="Escolha como os domingos de folga devem ser distribuídos"
        )
        
        # Alternância entre colaboradores
        alternancia_colaboradores = st.checkbox(
            "Alternar dias de folga entre colaboradores",
            value=True,
            help="Quando ativado, distribui os dias de folga de forma alternada entre os colaboradores 6x1"
        )
        
        # Configurações avançadas
        with st.expander("🔧 Configurações Avançadas"):
            max_dias_consecutivos = st.slider(
                "Máximo de dias consecutivos trabalhados",
                min_value=5,
                max_value=7,
                value=6,
                help="Máximo de dias que um colaborador pode trabalhar consecutivamente"
            )
            
            semanas_sem_domingo_max = st.slider(
                "Máximo de semanas sem domingo de folga",
                min_value=4,
                max_value=8,
                value=6,
                help="Máximo de semanas que um colaborador pode ficar sem folgar no domingo"
            )
        
        # Preparar configuração 6x1
        config_6x1 = {
            'folga_domingo_obrigatoria': folga_domingo_obrigatoria[0] == 'obrigatoria',
            ...
            if st.button("🚀 Gerar Escala", type="primary"):
                with st.spinner("Gerando escala..."):
                    ...
                    **⚙️ Configurações Aplicadas:**
                    - **Regra de Domingo**: {'1 domingo por mês' if config_6x1['folga_domingo_obrigatoria'] else '1 domingo a cada 7 semanas'}
                    - **Alternância**: {'Ativada' if config_6x1['alternancia_colaboradores'] else 'Desativada'}
                    - **Máximo dias consecutivos**: {config_6x1['max_dias_consecutivos']} dias
                    - **Máximo semanas sem domingo**: {config_6x1['semanas_sem_domingo_max']} semanas
                    """)
