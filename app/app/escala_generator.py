...
class GeradorEscala:
    ...
    def __init__(self, data_inicio: date, data_fim: date, feriados: pd.DataFrame, 
        ...
        Args:
            ...
            config_6x1: Configurações específicas para escalas 6x1
        """
        ...
        # Configurar regras 6x1 se fornecidas
        if config_6x1:
            self.regras_clt.configurar_regras_6x1(**config_6x1)
        ...
        # Controle de alternância para colaboradores 6x1
        self.colaboradores_6x1 = []
        self.controle_alternancia = {}
    ...
    def gerar_escala_completa(self, df_colaboradores: pd.DataFrame) -> pd.DataFrame:
        ...
        # Identificar colaboradores 6x1 para alternância
        colaboradores_6x1 = df_colaboradores[df_colaboradores['Tipo_Escala'].str.contains('6X1')]
        self.colaboradores_6x1 = colaboradores_6x1['Nome'].tolist()
        
        # Inicializar controle de alternância para colaboradores 6x1
        if self.regras_clt.config_6x1['alternancia_colaboradores'] and len(self.colaboradores_6x1) > 1:
            self._inicializar_alternancia_6x1()
    ...
    def _inicializar_alternancia_6x1(self):
        """Inicializa o controle de alternância para colaboradores 6x1."""
        for i, nome in enumerate(self.colaboradores_6x1):
            self.controle_alternancia[nome] = {
                'indice': i,
                'total_colaboradores': len(self.colaboradores_6x1),
                'offset_folga': i * 2,  # Deslocamento para alternar dias de folga
                'ultimo_dia_folga': None
            }
    
    def _obter_dia_folga_alternado_6x1(self, nome: str, data_atual: date, 
                                      ultimo_domingo_folga: date = None) -> bool:
        """
        Determina se um colaborador deve folgar em uma data específica com alternância.
        ...
        Returns:
            True se deve folgar, False caso contrário
        """
        if nome not in self.controle_alternancia:
            return False
        
        controle = self.controle_alternancia[nome]
        
        # Calcular dias desde o início do período
        dias_desde_inicio = (data_atual - self.data_inicio).days
        
        # Aplicar offset para alternar dias de folga
        dias_ajustados = dias_desde_inicio + controle['offset_folga']
        
        # Regra 6x1: folga a cada 7 dias (6 trabalho + 1 folga)
        deve_folgar = (dias_ajustados % 7) == 6
        
        # Verificar regras de domingo
        if data_atual.weekday() == 6:  # Domingo
            if self.regras_clt.config_6x1['folga_domingo_obrigatoria']:
                # 1 domingo por mês obrigatório
                deve_folgar = data_atual.day <= 7
            else:
                # 1 domingo a cada 7 semanas
                if ultimo_domingo_folga:
                    semanas_sem_domingo = (data_atual - ultimo_domingo_folga).days // 7
                    deve_folgar = semanas_sem_domingo >= self.regras_clt.config_6x1['semanas_sem_domingo_max']
        
        return deve_folgar
    
    def _gerar_escala_6x1_alternada(self, nome: str, tipo_escala: str, ultimo_domingo_folga: date = None) -> Dict[date, str]:
        """
        Gera escala 6x1 com alternância entre colaboradores.
        
        Args:
            nome: Nome do colaborador
            tipo_escala: Tipo da escala (M6X1, T6X1, N6X1)
            ultimo_domingo_folga: Último domingo de folga
            
        Returns:
            Dicionário com a escala do colaborador
        """
        escala = {}
        data_atual = self.data_inicio
        dias_trabalhados = 0
        ultimo_dia_folga = ultimo_domingo_folga
        
        # Determinar o status de trabalho baseado no tipo de escala
        if tipo_escala.startswith('M6X1'):
            status_trabalho = 'TRABALHO_MANHA'
        elif tipo_escala.startswith('T6X1'):
            status_trabalho = 'TRABALHO_TARDE'
        elif tipo_escala.startswith('N6X1'):
            status_trabalho = 'TRABALHO_NOITE'
        else:
            status_trabalho = 'TRABALHO_DIA'
        
        while data_atual <= self.data_fim:
            # Verificar se deve folgar baseado na alternância
            deve_folgar = self._obter_dia_folga_alternado_6x1(nome, data_atual, ultimo_domingo_folga)
            
            # Verificar se já trabalhou 6 dias consecutivos
            if dias_trabalhados >= self.regras_clt.config_6x1['max_dias_consecutivos']:
                deve_folgar = True
            ...
            if data_atual in self.feriados:
                ...
                dias_trabalhados = 0
                ultimo_dia_folga = data_atual
            else:
                escala[data_atual] = status_trabalho
                dias_trabalhados += 1
    ...
    def _gerar_escala_colaborador(self, colaborador: pd.Series) -> pd.DataFrame:
        ...
        # Gerar escala base para outros tipos
            ...
            # Verificar se deve usar alternância
            if (self.regras_clt.config_6x1['alternancia_colaboradores'] and 
                nome in self.colaboradores_6x1 and len(self.colaboradores_6x1) > 1):
                # Usar sistema de alternância personalizado
                escala_base = self._gerar_escala_6x1_alternada(nome, tipo_escala, ultimo_domingo)
                info_controle = {
                    'ultimo_domingo_folga': ultimo_domingo,
                    'domingos_folgados': 0,
                ...
                resultado = self.regras_clt.gerar_escala_base(
                    ...
                    ultimo_plantao, ultimo_domingo, nome
                )
