...
class RegrasCLT:
    ...
    def __init__(self):
        ...
        # Configurações para escalas 6x1
        self.config_6x1 = {
            'folga_domingo_obrigatoria': True,  # True: 1 domingo por mês, False: 1 domingo a cada 7 semanas
            'alternancia_colaboradores': True,  # Alternar dias de folga entre colaboradores
            'max_dias_consecutivos': 6,  # Máximo de dias consecutivos trabalhados
            'semanas_sem_domingo_max': 6  # Máximo de semanas sem folgar no domingo
        }
        
        # Controle de alternância para colaboradores 6x1
        self.controle_alternancia_6x1 = {}
    ...
    def gerar_escala_base(self, tipo_escala: str, data_inicio: date, data_fim: date, 
                         ...
                         ultimo_domingo_folga: date = None, colaborador_id: str = None) -> Dict[date, str]:
        """
        ...
        if tipo_escala.startswith('P_') or tipo_escala.startswith('I_'):
            ...
            resultado = self.tipos_escala[tipo_escala](data_inicio, data_fim, feriados, ultimo_domingo_folga, colaborador_id)
            # Retornar apenas a escala, não a tupla (escala, info_controle)
    ...
    def _regra_6x1_manha(self, data_inicio: date, data_fim: date, feriados: List[date], ultimo_domingo_folga: date = None, colaborador_id: str = None) -> Tuple[Dict[date, str], Dict[str, Any]]:
        """
        ...
        while data_atual <= data_fim:
            ...
            if data_atual.weekday() == 6:  # Domingo
                deve_folgar_domingo = False
                
                if self.config_6x1['folga_domingo_obrigatoria']:
                    # 1 domingo por mês obrigatório (primeira semana)
                    deve_folgar_domingo = data_atual.day <= 7
                else:
                    # 1 domingo a cada 7 semanas
                    deve_folgar_domingo = semanas_sem_domingo >= self.config_6x1['semanas_sem_domingo_max']
                
                if deve_folgar_domingo or dias_trabalhados >= self.config_6x1['max_dias_consecutivos']:
                    escala[data_atual] = "FOLGA"
                    domingos_folgados += 1
                ...
                if dias_trabalhados < self.config_6x1['max_dias_consecutivos']:
                    escala[data_atual] = "TRABALHO_MANHA"
                    dias_trabalhados += 1
                else:
                    escala[data_atual] = "FOLGA"
    ...
    def _regra_6x1_tarde(self, data_inicio: date, data_fim: date, feriados: List[date], ultimo_domingo_folga: date = None, colaborador_id: str = None) -> Tuple[Dict[date, str], Dict[str, Any]]:
        """
    ...
    def _regra_6x1_noite(self, data_inicio: date, data_fim: date, feriados: List[date], ultimo_domingo_folga: date = None, colaborador_id: str = None) -> Tuple[Dict[date, str], Dict[str, Any]]:
        """
    ...
    def configurar_regras_6x1(self, folga_domingo_obrigatoria: bool = True, 
                             alternancia_colaboradores: bool = True,
                             max_dias_consecutivos: int = 6,
                             semanas_sem_domingo_max: int = 6):
        """
        ...
        """
        self.config_6x1.update({
            'folga_domingo_obrigatoria': folga_domingo_obrigatoria,
            'alternancia_colaboradores': alternancia_colaboradores,
            'max_dias_consecutivos': max_dias_consecutivos,
            'semanas_sem_domingo_max': semanas_sem_domingo_max
        })
    
    def _obter_proximo_dia_folga_6x1(self, colaborador_id: str, data_atual: date, 
                                    ultimo_domingo_folga: date = None) -> date:
        """
        ...
        """
        if not self.config_6x1['alternancia_colaboradores']:
            # Sem alternância - usar regra padrão
            return data_atual + timedelta(days=6)
        
        # Inicializar controle de alternância se necessário
        if colaborador_id not in self.controle_alternancia_6x1:
            self.controle_alternancia_6x1[colaborador_id] = {
                'ultimo_dia_folga': ultimo_domingo_folga,
                'dias_trabalhados': 0,
                'semanas_sem_domingo': 0
            }
        
        controle = self.controle_alternancia_6x1[colaborador_id]
        
        # Verificar se é domingo
        if data_atual.weekday() == 6:  # Domingo
            # Regra de domingo baseada na configuração
            if self.config_6x1['folga_domingo_obrigatoria']:
                # 1 domingo por mês obrigatório
                if data_atual.day <= 7:  # Primeira semana do mês
                    return data_atual
            else:
                # 1 domingo a cada 7 semanas
                if controle['semanas_sem_domingo'] >= self.config_6x1['semanas_sem_domingo_max']:
                    return data_atual
        
        # Regra 6x1: 6 dias trabalho, 1 dia folga
        if controle['dias_trabalhados'] >= self.config_6x1['max_dias_consecutivos']:
            return data_atual
        
        # Calcular próximo dia de folga baseado no padrão 6x1
        dias_desde_ultima_folga = (data_atual - (controle['ultimo_dia_folga'] or data_atual)).days
        if dias_desde_ultima_folga >= 6:
            return data_atual
        
        # Se não for dia de folga, continuar trabalhando
        return None 